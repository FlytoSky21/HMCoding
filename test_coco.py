...
def compute_bpp(out_net):
    ...
    enhance_bpp = sum(torch.log(likelihoods).sum() / (-math.log(2) * num_pixels)
                      for likelihoods in out_net["enhance_likelihoods"].values()).item()
    total_bpp = base_bpp + enhance_bpp
    return base_bpp, enhance_bpp, total_bpp
...
def _evaluate(model, dataloader, class_names, img_size, iou_thres, conf_thres, nms_thres, verbose):
    ...
    for _, imgs, imgs_lr, targets in tqdm.tqdm(dataloader, desc="Validating"):
        ...
        with torch.no_grad():
            ...
            enhance_bpp += enhance_bpp_tmp
            total_bpp += total_bpp_tmp
...
def run():
    ...
    print(f"valid path is {valid_path}")
